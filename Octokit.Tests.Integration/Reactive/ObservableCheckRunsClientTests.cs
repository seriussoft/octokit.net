        ...
        public class TheCreateMethod
        {
            ...
            public async Task CreatesCheckRun()
            {
                using (var repoContext = await _github.CreateRepositoryContext(new NewRepository(Helper.MakeNameWithTimestamp("public-repo")) { AutoInit = true }))
                {
                    ...
                    var newCheckRun = new NewCheckRun("status", featureBranch.Object.Sha)
                    {
                        Status = CheckStatus.Queued,
                        Output = new CheckRunOutput("My output", "The summary goes here")
                        {
                            Images = new List<CheckRunImage>()
                            {
                                new CheckRunImage("banana", "https://imgur.com/banana")
                            }
                        }
                    };
